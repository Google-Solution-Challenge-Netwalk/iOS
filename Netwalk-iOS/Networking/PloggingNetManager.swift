...
class PloggingNetManager {
    ...
    func startPlogging(_ startPlogData: StartPloggingData,
        ...
        guard let jsonData = try? JSONEncoder().encode(startPlogData) else {
        ...
        URLSession.shared.dataTask(with: request) { data, res, err in
            ...
            if let safeData = data {
                do {
                    let decodedData = try JSONDecoder().decode(ActivityObject.self, from: safeData)
                    dump(decodedData)
    ...
    func stopPlogging(_ activity: Activity, completion: @escaping ()->()) {
        
        let urlKey = Bundle.main.getSecretKey(key: "REST_API_URL")
        
        guard let url = URL(string: "\(urlKey)/api/v1/act/update") else {
            print("URL Error")
            return
        }
